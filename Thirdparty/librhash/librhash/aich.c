 ...
 */
static void aich_chunk_table_extend(aich_ctx* ctx, unsigned chunk_num)
{
  ...
  if(index >= ctx->allocated) {
    ...
    ctx->chunk_table = realloc(ctx->chunk_table, new_size * sizeof(void*));
 ...
 */
static void aich_hash_tree(aich_ctx *ctx, unsigned char* result, int type)
{
 ...
 */
static void aich_process_block(aich_ctx *ctx, int type)
{
  assert(type != 0);
  assert(ctx->index <= ED2K_CHUNK_SIZE);
  ...
  if(ctx->index >= ED2K_CHUNK_SIZE || (type & AICH_PROCESS_FINAL_BLOCK)) {
    ...
    if(CT_INDEX(ctx->chunks_number) == 0) {
      aich_chunk_table_extend(ctx, ctx->chunks_number);
      if(ctx->error) return;
    }
    ...
    if(!(type & AICH_PROCESS_FINAL_BLOCK) || ctx->chunks_number == 0) {
      ...
      aich_hash_tree(ctx, pair[1], AICH_HASH_LEFT_BRANCH);
    }
    ...
    if(ctx->chunks_number > 0) {
      ...
      aich_hash_tree(ctx, pair[0], AICH_HASH_RIGHT_BRANCH);
    }
...
void aich_update(aich_ctx *ctx, const unsigned char* msg, size_t size)
{
  ...
  while(size > 0) {
    ...
    if(size >= block_left) {
      ...
      //IF_OPENSSL(ctx->sha1_length += block_left);
      msg  += block_left;
      size -= block_left;
      ctx->index += block_left;

      /* process a 180KiB-blok */
      aich_process_block(ctx, AICH_PROCESS_FLUSH_BLOCK);
      SHA1_INIT(ctx);
    } else {
      /* add to a leaf block */
      SHA1_UPDATE(ctx, msg, size);
      //IF_OPENSSL(ctx->sha1_length += size);
...
void aich_final(aich_ctx *ctx, unsigned char result[20])
{
  ...
  if(ctx->chunks_number == 0 && ctx->block_hashes == NULL) {
    ...
    u32_memswap(ctx->sha1_context.hash, 5);
#endif
#endif
  ...
  if((ctx->index % FULL_BLOCK_SIZE) > 0) {
    ...
    aich_process_block(ctx, ctx->block_hashes != NULL ? 
  ...
  if(ctx->chunks_number == 0) {
    ...
    aich_hash_tree(ctx, hash, AICH_HASH_LEFT_BRANCH);
  } else {
    if(ctx->index > 0) {
      /* process the last block of the message */
      aich_process_block(ctx, AICH_PROCESS_FINAL_BLOCK);
    }
    assert(ctx->chunks_number > 0);
    assert(ctx->block_hashes != NULL);

    aich_hash_tree(ctx, hash, AICH_HASH_FULL_TREE);
  }
