 ...
 */
void edonr256_init(edonr_ctx *ctx)
{
 ...
 * @param ctx context to initalize
 */
void edonr224_init(struct edonr_ctx *ctx)
{
 ...
 * @param ctx context to initalize
 */
void edonr512_init(edonr_ctx *ctx)
{
 ...
 * @param ctx context to initalize
 */
void edonr384_init(struct edonr_ctx *ctx)
{
...
/* Q256 macro was taken from eBASH submission */
...
/* Q512 macro was taken from eBASH submission */
 ...
 */
static void edonr256_process_block(unsigned hash[16], const unsigned *block, size_t count)
{
  while(1) {
 ...
 * @param block the message block to process
 */
static void edonr512_process_block(uint64_t hash[16], const uint64_t *block, size_t count)
{
  while(1) {
 ...
 */
void edonr256_update(edonr_ctx *ctx, const unsigned char *msg, size_t size)
{
  ...
  if(index) {
    ...
    edonr256_process_block(ctx->u.data256.hash, ctx->u.data256.message, 1);
    msg  += left;
    size -= left;
  }
  ...
  if(size >= edonr256_block_size) {
    if( IS_LITTLE_ENDIAN && IS_ALIGNED_32(msg) ) {
      ...
      edonr256_process_block(ctx->u.data256.hash, (unsigned*)msg, count);
      msg  += edonr256_block_size * count;
      size -= edonr256_block_size * count;
    } else {
      do {
        le32_copy(ctx->u.data256.message, msg, edonr256_block_size);
        edonr256_process_block(ctx->u.data256.hash, ctx->u.data256.message, 1);
  ...
  if(size) {
    le32_copy(ctx->u.data256.message, msg, size); /* save leftovers */
  }
}
 ...
 */
void edonr256_final(edonr_ctx *ctx, unsigned char* result)
{
  ...
  if(result) {
    ...
    le32_copy(result, ((char*)ctx->u.data256.hash) + off, ctx->digest_length);
  }
}
 ...
 * @param size length of the message chunk
 */
void edonr512_update(edonr_ctx *ctx, const unsigned char *msg, size_t size)
{
  ...
  if(index) {
    ...
    /* process partial block */
    edonr512_process_block(ctx->u.data512.hash, ctx->u.data512.message, 1);
    msg  += left;
    size -= left;
  }
  if(size >= edonr512_block_size) {
    if( IS_LITTLE_ENDIAN && IS_ALIGNED_64(msg) ) {
      ...
      edonr512_process_block(ctx->u.data512.hash, (uint64_t*)msg, count);
 ...
 * @param result calculated hash in binary form
 */
void edonr512_final(edonr_ctx *ctx, unsigned char* result)
{
