 ...
 */
static void ripemd160_process_block(unsigned *hash, const unsigned* X)
{
 ...
 */
void ripemd160_update(ripemd160_ctx *ctx, const unsigned char* msg, size_t size)
{
  ...
  if(index) {
    ...
    ripemd160_process_block(ctx->hash, (unsigned*)ctx->message);
    msg += left;
    size -= left;
  }
  while(size >= ripemd160_block_size) {
    ...
    if( IS_LITTLE_ENDIAN && IS_ALIGNED_32(msg) ) {
      ...
      le32_copy(ctx->message, msg, ripemd160_block_size);
      aligned_message_block = ctx->message;
    }

    ripemd160_process_block(ctx->hash, aligned_message_block);
 ...
 */
void ripemd160_final(ripemd160_ctx *ctx, unsigned char result[20])
{
  ...
  if(index>14) {
    ...
    ripemd160_process_block(ctx->hash, ctx->message);
    index = 0;
  }
  while(index < 14) {
    ctx->message[index++] = 0;
