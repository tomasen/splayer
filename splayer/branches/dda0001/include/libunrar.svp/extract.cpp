...
int CmdExtract::ExtractChunkInit(Archive &Arc, int HeaderSize, char *file)
{
	int HeaderType;

	DataIO.SetCurrentCommand('P');

	if (HeaderSize <= 0) {
		if (DataIO.UnpVolume) {
			if (!MergeArchive(Arc, &DataIO, false, 'X')) {
				return (-1);
			}
		}
		else {
	...
	*/
	if (Arc.GetHeaderType() != FILE_HEAD)
		Arc.SearchBlock(FILE_HEAD);
	do {
#if 0
		printf("%s %lld\n", Arc.NewLhd.FileName, Arc.NewLhd.FullUnpSize);
		printf("%lld\n", Arc.NewLhd.FullPackSize);
		printf("%x\n", Arc.NewLhd.Method);
#endif
		/* XXX: fix unicode */
		//if (strcasecmp(Arc.NewLhd.FileName, file) == 0)
		//	break;
		if (_stricmp(Arc.NewLhd.FileName, file) == 0){
			SVP_LogMsg3("Got Matched File %s | %s",Arc.FileName , file);
			//strcpy(Arc.FileName , file);
			break;
		}
	} while (Arc.SearchBlock(FILE_HEAD));

	if (Arc.GetHeaderType() != FILE_HEAD)
		return (-1);

	FileOffset = Arc.NextBlockPos - Arc.NewLhd.FullPackSize;
	BytesLeft = Arc.NewLhd.FullUnpSize; 
	Arc.Seek(FileOffset, SEEK_SET);
	...
	ExtractFile = strdup(file);

	return (0);
}

int CmdExtract::ExtractChunkClose(Archive &Arc)
{
...
int CmdExtract::ExtractChunkSeek(Archive &Arc, uint64_t offset, int cur)
{
	int n;
	uint64_t abs;

	if (cur)
		abs = (uint64_t)(DataIO.ReadPos + (int64_t)offset);
	else
		abs = offset;

	n = DataIO.UnpReadSeek(ExtractFile, abs);
	if (n != 0)
		return (n);

	BytesLeft = Arc.NewLhd.FullUnpSize - abs; 
	DataIO.ReadPos = abs;
#ifdef DEBUG
	printf("%s: ReadPos=%llu, BytesLeft=%llu\n", __func__, DataIO.ReadPos, BytesLeft);
#endif
	return (0);
}

int CmdExtract::ExtractChunk(Archive &Arc, char *buf, size_t len)
{
	int n; 

	DataIO.SetUnpackToMemory((byte *)buf, len);
	SVP_LogMsg3("UnstoreFile Method %x %d" , Arc.NewLhd.Method ,  len);
	if (Arc.NewLhd.Method == 0x30) {
		SVP_LogMsg3("UnstoreFile Start %d" , len);
		n = UnstoreFile(DataIO, len);
		SVP_LogMsg3("UnstoreFile Done %d" , n);
	}
	else {
		Unp->SetDestSize(len);
#ifndef SFX_MODULE
...
int64_t CmdExtract::UnstoreFile(ComprDataIO &DataIO,int64 DestUnpSize)
{
  Array<byte> Buffer(0x10000);
  int64_t Len; 
  int64_t len = 0;
  while (DestUnpSize > 0) // while (1)
  {
	Len = DestUnpSize > Buffer.Size() ? Buffer.Size() : DestUnpSize;
	unsigned int Code = DataIO.UnpRead(&Buffer[0], Len); 
	//uint Code=DataIO.UnpRead(&Buffer[0],Buffer.Size());
    if (Code==0 || (int)Code==-1)
      break;
    Code=Code<DestUnpSize ? Code:(uint)DestUnpSize;
    DataIO.UnpWrite(&Buffer[0],Code);
	if (DestUnpSize>=0) {//if (DestUnpSize>=0)
		DestUnpSize-=Code;
		len += Code;
	}
  }
