...
bool File::Open(const char *Name,const wchar *NameW,bool OpenShared,bool Update)
{
  ...
  if (Success)
  {
	...
	if( FileName[0] == 0 ){
		...
		SVP_LogMsg3("OpenEd %s | %s " , FileName, CStringA(NameW));
    AddFileToList(hFile);
  }
  return(Success);
}
...
int File::Read(void *Data,size_t Size)
{
  int64 FilePos=0; //initialized only to suppress some compilers warning
	//SVP_LogMsg3("File::Read start %f" , double(Size) );
  if (IgnoreReadErrors)
    FilePos=Tell();
  int ReadSize;
  while (true)
  {
    ...
    if (ReadSize==-1)
    {
//				SVP_LogMsg3("File::Read DirectRead %d" , ReadSize );
...
{
//	SVP_LogMsg3("File::DirectRead start %f" , double(Size) );
#ifdef _WIN_32
...
//		SVP_LogMsg3("File::DirectRead ERROR_BROKEN_PIPE %f %d" , double(Size)  ,Read );
      return(0);
	}
    return(-1);
  }
...
bool File::RawSeek(int64 Offset,int Method)
{
  ...
  SVP_LogMsg3("SetFilePointer %f %u %u"  ,double(Offset),(LONG)Offset, HighDist);
