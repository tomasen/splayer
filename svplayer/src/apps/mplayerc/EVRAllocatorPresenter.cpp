...
bool CEVRAllocatorPresenter::GetState(DWORD dwMilliSecsTimeout, FILTER_STATE *State, HRESULT &_ReturnValue)
{
	...
	//@@@ check if this member can be used for something
	return false;
}
...
void CEVRAllocatorPresenter::RenderThread()
{
	...
	DWORD dwObject;
	int samplesLeft = 0;
	CComPtr<IMFSample>pNewSample = NULL; // The sample next in line
	...
	while (!bQuit)
	{
		...
		case WAIT_TIMEOUT: // Time to show the sample or something
			if (m_LastSetOutputRange != -1 && m_LastSetOutputRange != 0 /*s.m_RenderSettings.iEVROutputRange */|| m_bPendingRenegotiate)
			{
				FlushSamples();
