/*
 * $Id: EVRAllocatorPresenter.cpp 931 2008-12-20 14:35:34Z casimir666 $
 *
...
CheckPointer(ppRenderer, E_POINTER);
	...
	*ppRenderer = NULL;
	...
	do
	{
		...
		COuterEVR *pOuterEVR = new COuterEVR(NAME("COuterEVR"), pUnk, hr, &m_VMR9AlphaBitmap, this);
...
bool CEVRAllocatorPresenter::GetImageFromMixer()
{
	...
	while (SUCCEEDED(hr))
	{
		...
		if( ( m_pSubPicQueue || m_pSubPicQueue2 ) ) //m_fUseInternalTimer &&
		{
			MFTIME		nsDuration;
			float		m_fps;
...
void CEVRAllocatorPresenter::RenderThread()
{
	...
	while (!bQuit)
	{
		...
		if (!1/*s.m_RenderSettings.iVMR9VSyncAccurate*/ && NextSleepTime == 0)
			NextSleepTime = 1;
...
//			TRACE("Sleep: %7.3f\n", double(AfxGetMyApp()->GetPerfCounter()-llPerf) / 10000.0);
		...
		case WAIT_TIMEOUT :
			...
			if (m_LastSetOutputRange != -1 && m_LastSetOutputRange != /*s.m_RenderSettings.iEVROutputRange*/ 0 || m_bPendingRenegotiate)
			{
				FlushSamples();
				RenegotiateMediaType();
				m_bPendingRenegotiate = false;
