	...
	: CFGManager(pName, pUnk)
{
	...
	if(s.onlyUseInternalDec || !s.useGPUCUDA || s.iDSVideoRendererType == VIDRNDT_DS_VMR7RENDERLESS){
		s.DXVAFilters = 0;
	}else{
		s.DXVAFilters = ~0;
	}
...
*/
	...
	pFGF = new CFGFilterInternal<CMpeg2DecFilter>(
		...
		(tra & TRA_MPEG1) ? MERIT64_ABOVE_DSHOW : MERIT64_UNLIKELY);
	...
	, m_armerit(MERIT64(MERIT_PREFERRED))
{
	...
	if(s.iDSVideoRendererType == VIDRNDT_DS_OLDRENDERER)
		...
		if(CMPlayerCApp::IsVista() && !s.bDisableEVR){
			m_transform.AddTail(new CFGFilterVideoRenderer(m_hWnd, CLSID_EVRAllocatorPresenter, L"EVR渲染器", m_vrmerit));
		}else{
