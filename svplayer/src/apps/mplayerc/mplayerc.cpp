...
#define ADDCMD(cmd) wmcmds.AddTail(wmcmd##cmd)
	...
	ADDCMD((ID_VIEW_FULLSCREEN, VK_RETURN, FVIRTKEY|FALT|FNOINVERT, _T("切换全屏"), 0, wmcmd::MUP));
...
void CMPlayerCApp::Settings::UpdateData(bool fSave)
{
	...
	else
	{
		...
		AudioBoost = (float)pApp->GetProfileInt(ResStr(IDS_R_SETTINGS), ResStr(IDS_RS_AUDIOBOOST), 70);
